...
def find_thresholds(filename,model_directory):
    ...
    result = differential_evolution(optim_genetics(t, y, classes), bounds=bounds, disp=True, init=population)
    print(result)
...
def train_part(model,dataset,loss,opt):
    ...
    with mytqdm(dataset) as pbar:
        for i, (x, t, l) in enumerate(pbar):
            ...
            # x.shape (128, 12, 1, 8192) | l.shape (128, 12)
            # y.shape (128, 26) | p.shape (128, 26)
            y, p = model(x, l)
            #p = torch.sigmoid(y)
